...
- **Packaging**: the package is named **outlay** (decided 2026-07-05), home repo https://github.com/alechemy/outlay; `outlay@1.4.0` is published to npm (tagged `latest`, with the `./svg`/`./font`/`./render` exports) and released on GitHub; publishing and pushing remain Alec-only. Releases run through `.github/workflows/release.yml` on tag push: it publishes with CI's `NPM_TOKEN` and builds the GitHub release notes from the `## <version>` CHANGELOG.md section of the *tagged commit* — write the CHANGELOG entry before `npm version`, or the notes step fails (see TODO.md). `validateTree` (`src/validate.ts`, exported from the index) is the development-time input validator: errors for unsupported vocabulary (duplicate ids, percentages, bad enums/tracks), warnings for documented CSS divergences (block auto-height, margin collapse, grid baseline); tests in `tests/validate.test.ts` (`npm run test:validate`). CI is `.github/workflows/ci.yml`: typecheck, build, fixture suite, trace/validate/api/text/svg/font/html/render/testing tests, package smoke test, example package, both browser verifiers, bench.
...
- **Rendering track (2026-07-09, the flagship positioning)**: the prove-or-kill spike passed — headless rendering is the lead use case (Grid + Chromium-exact + pure sync JS is unique; Yoga/Satori have no Grid, Taffy is async WASM), and the README now leads with a competitor table and the generated OG card; the testing use case is demoted to a secondary section and deliberately not expanded. New subpaths, each with its own test script: `outlay/svg` (`src/svg.ts` — `renderToSvg` styled painter with per-line `textLength` text and CSS half-leading baselines when given font ascent/descent, `renderDebugSvg` depth-colored box visualizer with dashed grid-track outlines from `DebugTrace.gridLayouts`; `tests/svg.test.ts`), `outlay/font` (`src/font.ts` — zero-dependency TTF/OTF `DataView` parser over head/cmap-4-and-12/hhea/hmtx → `parseFont`/`wordAdvance`/`spaceAdvance`/`measureText`; advances are unshaped/unkerned, matching Chromium with `font-kerning: none` to 0.015px, verified in `tests/font.test.ts` against committed ground truth captured by `scripts/capture-font-ground-truth.ts` with the committed OFL `tests/assets/Inter-Regular.ttf`; default shaping diverges up to ~10px on kerning-heavy words — documented, not chased), and `outlay/render` (`src/render.ts` — `htmlToSvg`/`htmlToLayout`, the Satori shape with Grid: `parseHTML` gained an opt-in second parameter enabling text nodes, inherited text properties, and solid-color paint vocabulary, byte-for-byte unchanged without it; text-only elements become block `measureContent` leaves; mixed text+element content throws; `tests/render.test.ts`). `outlay/text` gained `breakLines` (word-range line breaker sharing `measureFromAdvances`' quantized fit test so painted lines always match measured layout). Raster output is deliberately out of scope — SVG only, PNG is the caller's rasterizer.
- **Demos**: eight core demos plus the Text-driven layout demo are built, all linked from the landing page (`pages/demos/index.html`). The newest is the OG-Image Generator (`pages/demos/og-image/generate.ts`, `npm run demo:og-image`, commits `card.svg`, linked from the README lead): a 1200×630 CSS Grid card (5fr/2fr/2fr columns, column+row spans, fr rows, tagline wrapped at the fr-resolved width, nested flex stat tiles) measured from the committed Inter TTF via `outlay/font` and painted by `outlay/svg` in ~3ms, no browser anywhere in the pipeline. Server-Side Layout (`server-layout/generate.ts`) now runs on `renderDebugSvg` with grid-track overlays. Three have committed headless verify scripts: Layout Explorer (`explorer.html`, flex + grid controls incl. aspectRatio/percentages/fit-content/keyword min-max/space-evenly, `verify:explorer` 61/61), HTML Playground (`html-playground.html` — textarea → `parseHTML` → solver pane vs. injected live HTML with the match badge, styled `HTMLParseError` state with `.path`; `verify:html-demo` 6/6 on port 5201; browser pane forces content-box so injected markup reflects real browser defaults), and Text-driven layout (`text-layout.html`, `verify:text-demo` 11/11), which wires the Pretext adapter (`pages/demos/pretext-adapter.ts`) into a live card grid. The other five were verified live via the chrome-devtools MCP (navigate, interact, screenshot, console): Animated Transitions (`transitions.html` — solver-computed before/after frames tweened with rAF), Drag-and-Drop Reorder (`drag-reorder.html` — grid re-solved per pointer-move, `transform` glide), Virtual Scroll (`virtual-scroll.html` — 10k rows solved once, Pretext row heights, ~16 DOM nodes at any scroll), Nested Dashboard (`dashboard.html` — ~110-node flex+grid tree with a solver-vs-native-CSS toggle matching at ≤0.01px, responsive column reflow), and Server-Side Layout (`pages/demos/server-layout/generate.ts`, `npm run demo:server-layout`, commits `example-output.svg`). Shared box palette in `pages/demos/palette.ts`. Vite resolves the vendored `pretext/` tree from source via a plugin in `vite.config.ts` (no build step; import `@chenglou/pretext`).
...
- **Testing example (secondary use case)**: `examples/layout-assertions/` — a self-contained vitest package (own `package.json`, depends on the root via `"outlay": "file:../.."`) demonstrating layout assertions in component tests. It imports `sweep`/`assertNoOverlaps`/`overflowsX` from `outlay/testing` (the shipped primitives); its local `helpers.ts` keeps only what the subpath doesn't ship (`boxOf`, `assertContained`, `range`) plus a Node-safe greedy text measurer (`text-metrics.ts` + `word-metrics.json`, mirroring the fixture runner). Run with `npm run test:example` (builds `dist/`, installs the example, runs vitest). Root README has a "Testing layouts" section, deliberately positioned after headless rendering; per the 2026-07-09 traction decision, do NOT expand testing DX (no custom matchers, no failure-artifact overlays, no computed-styles→LayoutNode bridge) unless a concrete user asks.
