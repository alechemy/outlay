...
- **Width-dependent text one level deep (text inside a flex/grid item that is itself a flex/grid container).** Tiers 26–27 cover text as a *direct* child of the sized flex/grid container, where the reorder measures it at its resolved width. When a text leaf sits inside a *nested* container (text⊂flex⊂grid, text⊂grid⊂flex), the outer container's row/cross contribution is computed by `computeIntrinsicContentSize`, which recurses through the nested container measuring the text at unconstrained min/max-content width rather than the item's resolved width — so nested text over-wraps. A trial tier 28 confirmed this; it needs `computeIntrinsicContentSize` to lay the subtree out at the resolved width (a degenerate-layout pass, Pretext pattern 1) rather than a contribution reorder, so it was deferred as an explicit gap. Constant-size content nests correctly (tiers 23, 25) because its measurement ignores width.
- **`repeat(auto-fill/auto-fit)` on a grid whose inline size is *derived* (a stretched flex item with no explicit `width`), unverified and buggy.** Discovered building the dashboard demo (2026-07-05). When the grid's own width is definite (explicit `width`, tier 24) the auto-repeat track count and row count are correct. When the width instead comes from stretching inside a flex parent, the auto-fill column count is resolved before the inline size is known (≈0 → one column), so the grid manufactures one implicit row per item and reports a far-too-tall height, even though the items still lay out across the correct number of columns. No fixture covers derived-width auto-repeat, so this is unlocked. Workaround in `dashboard.ts`: compute the column count in JS from the available width and emit `repeat(N, "1fr")` (explicit counts lay out correctly when stretched). A future tier should generate auto-repeat grids sized by a flex parent.
