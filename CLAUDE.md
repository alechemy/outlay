...
- **Demos**: the Layout Explorer (`pages/demos/explorer.html`, flex + grid controls, `verify:explorer` 50/50) and the Text-driven layout demo (`pages/demos/text-layout.html`, `verify:text-demo` 11/11). The text demo wires the Pretext adapter (`pages/demos/pretext-adapter.ts`) into a live card grid and checks the solver against the browser reference across widths. Vite resolves the vendored `pretext/` tree from source via a plugin in `vite.config.ts` (no build step; import `@chenglou/pretext`). `DEMOS_PROMPT.md` specifies the remaining demos.
