...
async function smokeJavaScriptEsm(tarballPath: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      "import { parseFont } from 'outlay/font';",
      ...
      "",
      "const svg = renderDebugSvg(root, result);",
      "if (!svg.startsWith('<svg') || !svg.includes('>root</text>')) throw new Error('bad debug svg');",
      "",
      "let fontThrew = false;",
      "try { parseFont(new Uint8Array([0x74, 0x74, 0x63, 0x66, 0, 0, 0, 0, 0, 0, 0, 0])); } catch { fontThrew = true; }",
      "if (!fontThrew) throw new Error('parseFont accepted a TTC header');",
      "",
      "const rendered = htmlToSvg('<div style=\"display: grid; width: 40px; height: 20px; grid-template-columns: 1fr 1fr\"><div style=\"background: #123456\"></div><div></div></div>', { fonts: {} });",
      "if (!rendered.includes('fill=\"#123456\"') || !rendered.includes('width=\"20\"')) throw new Error('bad htmlToSvg output');",
      "",
...
async function smokeTypeScript(tarballPath: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      "import { measureFromAdvances, breakLines, type MeasureContent } from 'outlay/text';",
      "import { sweep, assertNoOverlaps, overflowsX, overflowsY } from 'outlay/testing';",
      "import { renderToSvg, renderDebugSvg, type SvgBoxStyle } from 'outlay/svg';",
      "import { parseFont, measureText, type FontMetrics } from 'outlay/font';",
      "import { htmlToSvg, htmlToLayout, type HtmlToSvgOptions } from 'outlay/render';",
      "",
      ...
      "",
      "renderDebugSvg(root, result) satisfies string;",
      "const style: SvgBoxStyle = { fill: '#fff', radius: 4 };",
      "renderToSvg(root, result, { style: () => style }) satisfies string;",
      "breakLines([1, 2], { spaceWidth: 1 })(10) satisfies { start: number; end: number; width: number }[];",
      "parseFont satisfies (data: Uint8Array) => FontMetrics;",
      "measureText satisfies (font: FontMetrics, text: string, opts: { size: number; lineHeight: number }) => MeasureContent;",
      "const renderOpts: HtmlToSvgOptions = { fonts: {} };",
      "htmlToSvg('<div style=\"width: 10px; height: 10px\"></div>', renderOpts) satisfies string;",
      "htmlToLayout('<div style=\"width: 10px; height: 10px\"></div>', renderOpts).tree satisfies LayoutNode;",
      "",
    ].join("\n"),
  );
